...
describe("Complex VaultDiamond Tests", async function () {
  ...
  before(async function () {
    ...
    result = await diamondLoupeFacet.facetFunctionSelectors(erc1155Facet.address);
    ...
    response = await diamondLoupeFacet.facetFunctionSelectors(diamondLoupeFacet.address)
   console.log("diamondLoupeFacet",response)
    ...
    const hedgePositionsSelector = loanFacet.interface.getSighash(
      "hedgePositions()"
    );
    ...
    const diamondLoupeFacetAddress = await diamondLoupeFacet.facetAddress(
     ...
     // using diamondLoupeFacet to find ownershipFacetAddress
      //owner()
      const ownerSelector = await ownershipFacet.interface.getSighash(
        "owner()"
      );
    ...
    const createVaultResponse = await vm.createVault(
      ...
      diamondAddress,
      [diamondCutFacetAddress,diamondLoupeFacetAddress,ownershipFacetAddress,
      loanFacetAddress,swapFacetAddress
         ,erc1155FacetAddress
    ],
      diamondInitAddress
    );
    ...
    if (Object.keys(pairs).length !== 0) {
      ...
      // the token contracts
        ...
        console.log(`Succesfully minted ${(await pairs["USDC"].balanceOf(deployer)).toString()}  USDC`)
        console.log(`Succesfully minted ${(await pairs["WETH"].balanceOf(deployer)).toString()} WETH`)
        console.log(`Succesfully minted ${(await pairs["WBTC"].balanceOf(deployer)).toString()} WBTC`)
    ...
    //Note : original vault ko nextID is initialized..

    //=> here we attach the facets to the initialized Vault.
  ...
  it("heyy",()=>{
    console.log("heyyy function bhitra");
    console.log("hey")
    console.log("h")
  })
  it("check loanfacet works",()=>{
    console.log("heyyy function bhitra");
    console.log("hey")
    console.log("h")
  })
  ...
  it("check the balance of owner Complex", async () => {
    expect(
      BigInt(await pairs["WETH"].balanceOf(deployer)) >= BigInt(10 ** 16)
    ).to.be.true;
    expect(
      BigInt(await pairs["WBTC"].balanceOf(deployer)) >= BigInt(10 ** 16)
    ).to.be.true;
    expect(
      BigInt(await pairs["USDC"].balanceOf(deployer)) >= BigInt(10 ** 16)
    ).to.be.true;
  ...
  it("add and remove liquidity", async () =>{
    ...
    expect(
      BigInt(await pairs["USDC"].balanceOf(vaultAddress)) >= amt
    ).to.equal(true);
  ...
  it("Take and Repay Loan", async function () {
    let prajanTest = new Date().getTime();
    console.log("prajanTest", prajanTest)

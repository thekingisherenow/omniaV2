...
describe("prajan", function () {
    ...
    it("addLiquidity twice and withdrawing again. ", async () => {
        ...
        await expect(await (await vault.balanceOf(owner.address, 0)).toString()).to.equal("0")
        ...
        await tx.wait(1)
        await expect(await (await vault.balanceOf(owner.address, 0)).toString()).to.equal(
            USDPRICE.toString()
        )
        ...
        await tx.wait(1)
        //2. addLiquidity
        const addLiquidityResponse = await vault.addLiquidity(ETHPrice, pairs.WETH.address)
        await addLiquidityResponse.wait(1)

        //check the balance of the deployer before and afer liqudiity.

        tx = await vault.withdrawLiquidity(BigInt(10 ** 14) * BigInt(1), pairs.USDC.address)
        await tx.wait(1)
    })
    ...
    it("swap test", async () => {
        ...
        //
        const approveResponse = await pairs.WBTC.approve(vault.address, ethers.constants.MaxUint256)
        await approveResponse.wait(1)
        const response = await vault.swap(pairs.WBTC.address, pairs.USDC.address, PRICE)
    })
    ...
    it("getUsdLogic", async () => {
        let price = 1000
        console.log("wbtcResponse", await vault.getUSDValue(pairs.WBTC.address, price))
        console.log("wethResponse", await vault.getUSDValue(pairs.WETH.address, price))
        console.log("usdResponse", await vault.getUSDValue(pairs.USDC.address, price))
    })
